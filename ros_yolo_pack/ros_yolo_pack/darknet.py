...
solib_path = packet_path + "yolo/libdarknet.so"
...
def load_network3(config_file, name_file, weights, batch_size=1):
...
def load_network2(config_file, data_file, weights, batch_size=1):
    network = load_net_custom(
        config_file.encode("ascii"),
        weights.encode("ascii"), 0, batch_size)
...
if os.name == "posix":
    ...
    lib = CDLL(solib_path, RTLD_GLOBAL)
elif os.name == "nt":
    cwd = os.path.dirname(__file__)
